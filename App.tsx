...
const VideoSelectionModal: React.FC<{
    ...
    const handleFavoritesResetClick = () => {
        if (window.confirm('お気に入りをすべてリセットしますか？この操作は元に戻せません。')) {
            onResetFavorites();
        }
    };

    const handleUserVideosResetClick = () => {
        if (window.confirm('追加した動画をすべてリセットしますか？この操作は元に戻せません。')) {
            onResetUserVideos();
        }
    };
...
const App: React.FC = () => {
  ...
  const initialVideoIds = useMemo(() => Array.from(new Set([
    ...
    '9f6_hQtkQsA', '9JQFov6GazM', '8wKDBiD4XsA', 'uXhyoPjNmaw', 'iqRMeq9ajZE', 'fxBWfB1LLlU', 'XTSgskSDKEw',
  ...
  const [videoId, setVideoId] = useState(initialVideoIds[0]);
  ...
  const handleSelectNewVideo = (id: string) => {
    if (videoId !== id) {
      setVideoId(id);
      // Attempt to autoplay with sound. Might be blocked by browser policies.
      setIsMuted(false);
    }
  };
  ...
  const embedUrl = `https://www.youtube.com/embed/${videoId}?autoplay=1&mute=${isMuted ? 1 : 0}&rel=0&playsinline=1`;
    ...
    <main className="bg-pink-50 dark:bg-rose-900 min-h-screen flex flex-col items-center justify-start p-4 sm:p-6 lg:p-8 font-sans transition-colors duration-300">
      <div className="w-full max-w-5xl mx-auto">
        ...
        <div className="w-full max-w-4xl mx-auto mb-6">
            <h2 className="text-2xl font-bold text-rose-800 dark:text-rose-200 mb-4">お気に入り</h2>
            {isLoading ? (
